...
# Set session
def set_session(filename):
    if not hasattr(thread_local, 'session'):
        thread_local.session = fitz.open(filename)
    else:
...
def process_all_images(filename, vectors):
    with concurrent.futures.ThreadPoolExecutor(max_workers=8, initializer=set_session, initargs=[filename]) as executor:
        result = executor.map(process_image, vectors)
